...
class GoogleDriveHelper:
    ...
    def clone(self, link):
        ...
        if meta.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
            ...
            r = requests.get('https://shrinkme.io/api?api=826ea316e7a34f7ecca236373b823448f8747309&url=' + self.__G_DRIVE_DIR_BASE_DOWNLOAD_URL.format(dir_id)).text
            y = json.loads(r)
            url = y['shortenedUrl']
            LOGGER.info(f" drive77 u :{url}")
            msg += f'<a href="{url}">{meta.get("name")}</a>' \
                   f' ({get_readable_file_size(self.transferred_size)})'
            if INDEX_URL is not None:

                url = requests.utils.requote_uri(f'{INDEX_URL}/{meta.get("name")}/')
                r = requests.get('https://shrinkme.io/api?api=826ea316e7a34f7ecca236373b823448f8747309&url=' + url).text
                y = json.loads(r)
                url = y['shortenedUrl']
                LOGGER.info(f" clone77 u :{url}")
            ...
            try:
                ...
                LOGGER.error(err)
                return err
            r = requests.get('https://shrinkme.io/api?api=826ea316e7a34f7ecca236373b823448f8747309&url=' + self.__G_DRIVE_BASE_DOWNLOAD_URL).text
            y = json.loads(r)
            url = y['shortenedUrl']
            LOGGER.info(f" clone777 u :{url}")
            msg += f'<a href="{url}">{file.get("name")}</a>'
            try:
                ...
                if INDEX_URL is not None:
                    ...
                    y = json.loads(r)
                    urlx = y['shortenedUrl']
                    LOGGER.info(f" clone779 u :{urlx}")
                    msg += f' | <a href="{urlx}"> Index URL</a>'
            except TypeError:
                pass
        return msg
